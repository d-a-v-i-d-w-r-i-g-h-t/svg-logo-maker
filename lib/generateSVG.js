...
function generateSVG(data) {
  ...
  switch(data.shape) {
    ...
    case 'circle':
      const circle = new Circle(data.shapeColor, data.text, data.textColor, data.font, data.height, data.width);
      svgContent = circle.render();
      break;

    case 'square':
      const square = new Square(data.shapeColor, data.text, data.textColor, data.font, data.height, data.width);
      svgContent = square.render();
      break;

    case 'triangle':
      const triangle = new Triangle(data.shapeColor, data.text, data.textColor, data.font, data.height, data.width);
